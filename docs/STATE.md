...
- **Code-diff-as-video mode — BUILT (POC, backend / teaching track):** `codediff.ts` + `assets/codediff-player.html` + `cli-codediff.ts`. Takes a file BEFORE/AFTER a change → `lineDiff()` (LCS → ordered hunks) → `narrateHunks()` (Claude gives the *why* of each hunk + a title, French or English) → a self-contained dark editor **replays the diff as a typewriter animation** (per-hunk: highlight+remove old lines in red, type new lines char-by-char in green, syntax-highlighted, line numbers) with **synchronized subtitles** (the LLM's why, chip "POURQUOI n/N"), ending on a "✓ appliqué" state. Recorded via the same Playwright engine → webm+gif. Verified on a bug-fix sample (off-by-one + divide-by-zero guard) → 2 clean hunks, spot-on French narration. Same spine as the UI engine (deterministic replay + Claude), different surface. Next ideas: multi-file hops, LLM-chosen teaching order, TTS voice-over, intra-line diff.
