...
- **Review money-shot = pitch centerpiece (Statement One)** — `moneyshot.ts`: reintroduce a drift in `sample-app/components.css` → `git.ts→render.ts→vlm.ts` verdict → `report.ts buildReportHtml` → screenshot to `.drift-cache/moneyshot/review-report.png`. Live verdict: **`accidental_regression` 97%**, reasoning names `--space-md`/`--color-brand`/the 4px scale, `proposed_diff` restores both to tokens — hits all 3 track beats (classify · reason-in-tokens · draft-fix). `report.ts` rebranded **Drift→Scenario + flame `#FF5A1F`**. Re-run: reintroduce the drift (padding→`14px`, bg→`#3B82F6`), `DRIFT_OPEN=0 npx tsx src/moneyshot.ts`, then `git checkout -- sample-app/components.css`.
