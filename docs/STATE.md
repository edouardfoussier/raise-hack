...
- **Feasibility notes for the roadmap:** scrolling during a recording ✔ (the `scroll` step); simulating the exact iOS keys being pressed ✔ (key-press highlight). A **backend "code-diff-as-video"** mode (type a file from its BEFORE into its AFTER diff, LLM narrates the *why*, hop across multiple files) is feasible with the same before/after+LLM engine — a Monaco/CodeMirror typewriter replay + TTS/caption track; parked as a future track (teaching / senior→junior knowledge transfer).
